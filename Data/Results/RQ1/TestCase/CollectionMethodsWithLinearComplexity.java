...
class A {
  ...
  void foo() {
    log.info("Queue contains " + queue.size() + " elements"); // Noncompliant [[sc=40;ec=44]] {{This call to "size()"
                                                              // may be a performance hot spot if the collection is
                                                              // large.}}
  }
}
...
class ArrayListUsage {
  ...
  void arrayList(Object a, String resClasspathPath) {
    ...
    if (resourceList.contains(resClasspathPath)) {// 新增
      // Already in the list e.g. same resource in different
      // archives...
      return false;
    }
  }
...
class Assignments {
  ...
  private List<Long> paymentListIdList = new ArrayList<>();
  private List<String> paymentListIdList2 = new ArrayList<>();

  void f(Object a) {
    list1.contains(a);// Noncompliant
    list2.contains(a); // Noncompliant
    list3.contains(a);//不检查无法确定的类型 除非是final或者private
    list7.contains(a);//compliant
    list1.remove(a);//compliant
    list1 = new ArrayList();
    list2 = new ArrayList();
    list1.contains(a); // Noncompliant
    list2.contains(a); // Noncompliant
    ...
    paymentListIdList.contains(a);//compliant
    paymentListIdList.remove(a);//compliant
    paymentListIdList2.contains(a);//compliant
    paymentListIdList2.remove(a);//compliant
  }
}
