...
class A {
  ...
  int foo() {
    ...
    if(cal.get(Calendar.MONTH) == foo()){};
    if(cal.get(Calendar.MONTH) == 12){}; // Noncompliant {{"12" is not a valid value for "MONTH".}}
    if(cal.get(Calendar.DAY_OF_MONTH) != 11){};
    if(cal.get(Calendar.DAY_OF_MONTH) != foo()){};
    if(cal.get(Calendar.DAY_OF_MONTH) != 32){}; // Noncompliant {{"32" is not a valid value for "DAY_OF_MONTH".}}
    if(31 == d.getDate()){};
    if(foo() == d.getDate()){};
    if(d.getDate() == 32){};
//    if(32 == d.getDate()){}; // Noncompliant {{"32" is not a valid value for "getDate".}}
    if(d1.getSeconds() == -1){};// Noncompliant {{"-1" is not a valid value for "getSeconds".}}
    if(calendar.get(Calendar.DST_OFFSET) == 0){};

    //获取昨日日期
    Calendar calendar = Calendar.getInstance();
    calendar.set(Calendar.HOUR_OF_DAY, -24);// Noncompliant {{"-24" is not a valid value for setting "HOUR_OF_DAY".}}
  }
}
